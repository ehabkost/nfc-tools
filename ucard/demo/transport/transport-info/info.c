...
on_card_presented (struct ucard *ucard, struct ucard_application *ucard_application)
{
    ...
    if (ucard_value_file_get_value (ucard, ucard_application, COUNTER_FILENO, COUNTER_RW_KEYNO, master_key, &value) < 0) goto error;
    printf("Il reste %d tickets\n",value);
    if (ucard_read_data(ucard, ucard_application, COUNTER_FILENO + 1, COUNTER_RW_KEYNO, master_key,0, sizeof(lastuse), lastuse) < 0) goto error;
    printf("Derniere utilisation: %s\n",lastuse);
    if (ucard_read_data(ucard, ucard_application, COUNTER_FILENO + 2, COUNTER_RW_KEYNO, master_key,0, sizeof(lastzone), lastzone) < 0) goto error;
    printf("Derniere zone: %s\n",lastzone);
